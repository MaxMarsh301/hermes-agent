...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            ("POST", "/v1/uploads", self._handle_upload),
            ("DELETE", "/v1/uploads/{file_id}", self._handle_delete_upload),
            ("POST", "/v1/runs", self._handle_runs),
            ("GET", "/v1/runs/{run_id}", self._handle_get_run),
            ("GET", "/v1/runs/{run_id}/events", self._handle_run_events),
            ("GET", "/v1/runs/{run_id}/artifacts", self._handle_run_artifacts),
            ...
            try:
                ...
                with self._profile_scope(request_profile):
                    agent = self._create_agent(
                        ...
                        event_callback=None if synthesize_only else _agent_event_cb,
                        gateway_session_key=gateway_session_key,
                        route=route,
                        synthesize_only=synthesize_only,
                    )
                ...
                # block below never fires — issue #15561).
                    ...
                    if synthesize_only:
                        failure_event["error"] = _SYNTHESIS_PRIVATE_OUTPUT_ERROR
                    self._emit_run_event(run_id, failure_event)
                    self._set_run_status(
                        run_id,
                        "failed",
                        error=error_msg,
                ...
                # Ordinary provider errors stay status-only and redacted. Private
                # synthesis publishes only the fixed contract literal expected by
                # the signed continuation consumer, never provider/request text.
                if synthesize_only:
                    failure_event["error"] = _SYNTHESIS_PRIVATE_OUTPUT_ERROR
                self._emit_run_event(run_id, failure_event)
            finally:
